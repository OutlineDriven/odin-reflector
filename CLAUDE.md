...
- **Rule:** On successful `tool_result` side-effect reviews (code changes and successful `bash`), return a `content` override carrying the verdict + opinion; a FAIL verdict additionally sets `isError: true`, which the extensions tool wrapper (`ExtensionToolWrapper`) rethrows as a failed tool call — blocking the edit/command. PASS/UNCERTAIN stay advisory (content only). On tool error paths, send diagnostics with `pi.sendMessage()` and return `undefined`; the original failed tool call already blocks.
  **Why:** OMP rethrows tool errors, so error results cannot carry feedback; on the success path, `isError: true` is the only lever that turns an advisory FAIL into a hard block. Overriding a genuine error result would corrupt the harness error path instead of preserving the original failure and can drop the diagnostic.

- **Rule:** The OMP extension is stateless — no `FailTracker`, no `appendEntry` FAIL entries, no `session_start` replay. `tool_result` side-effect reviews inject their verdict + opinion inline via `codeReviewResponse` for every verdict (PASS included); a FAIL additionally sets `isError: true` so the harness blocks that edit/command (fail-open — only FAIL blocks, never PASS/UNCERTAIN). This is an OMP-only divergence for per-tool code-change reviews: the Python plugin's per-tool FAIL stays advisory (`hookSpecificOutput.additionalContext`), so only its holistic Stop review blocks.
  **Why:** Per-tool reviews surface inline, but on OMP a FAIL must not be ignorable — `ExtensionToolWrapper` honors a success-path `isError: true` by rethrowing the result as a failed tool call, forcing remediation at the point of failure. The Python plugin keeps per-tool FAIL advisory by design (its holistic Stop review is the gate), so the two surfaces intentionally differ on per-tool enforcement.
